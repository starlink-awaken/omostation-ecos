...
STATE_HISTORY_LIMIT = 20  # 单节点保留的 state_history 条数上限
...
def _substantive(node: dict) -> dict:
    """节点里由 KEMS 推导出的部分, 不含每次运行必然变动的时间戳字段.

    created / state_history 是运行痕迹。让它们参与「是否变化」的判断, 会让每次
    --write 都产出全量 diff (实测 71 个节点里 22 个纯属噪声), 于是 diff 不再
    等于真实漂移。把「有没有跑过」与「内容变没变」分开, --write 的 diff 才可读。
    """
    return {k: v for k, v in node.items() if k not in {"created", "state_history"}}


def _carry_over_run_stamps(node: dict, fp: Path) -> bool:
    """把既有节点的时间戳字段继承进新节点; 返回「实质内容是否变化」."""
    if not fp.exists():
        return True
    try:
        previous = yaml.safe_load(fp.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, yaml.YAMLError):
        return True
    if not isinstance(previous, dict):
        return True
    node["created"] = previous.get("created") or node["created"]
    if _substantive(previous) == _substantive(node):
        # 实质未变: 连 state_history 一并保留 ⇒ 输出与既有文件逐字节一致
        node["state_history"] = previous.get("state_history") or node["state_history"]
        return False
    history = list(previous.get("state_history") or [])
    history.append(node["state_history"][-1])
    node["state_history"] = history[-STATE_HISTORY_LIMIT:]
    return True


def write_nodes(nodes: list[dict]) -> tuple[int, int]:
    """写入投影节点, 返回 (节点总数, 实质变化数).

    源未变时重复运行必须逐字节一致, 否则 --write 的 diff 无法用来判断漂移。
    """
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    changed = 0
    for n in nodes:
        fp = _node_path(n)
        if _carry_over_run_stamps(n, fp):
            changed += 1
        fp.write_text(
            _NODE_HEADER + yaml.safe_dump(n, allow_unicode=True, sort_keys=False),
            encoding="utf-8",
        )
...
def main() -> int:
    ...
    if not nodes:
        ...
        return 2
    if args.write:
        total, changed = write_nodes(nodes)
        print(
            f"✅ {total} 投影节点已写入 {OUT_DIR.relative_to(ECOS_ROOT)}"
            f"（实质变化 {changed}）"
        )
        return 0
    if args.freshness:
