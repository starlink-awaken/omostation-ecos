...
def format_brief(topology: dict, health: dict, debts: dict, scripts_count: int) -> str:
    ...
    lines.append("ecos whoami --json")
    lines.append("")
    lines.append("# 会话简报 (启动 Step 0)")
    lines.append("ecos brief")
    lines.append("")
    lines.append("# 一键治理")
    lines.append("ecos-constraint patrol")
    lines.append("```")
...
def main():
    ...
    print(f"\n{'-' * 56}")
    print("  Agent 启动: ecos brief --force")
    print("  一键治理: ecos-constraint patrol")
    print("  全域演练: make chaos-drill-strict")
    print(f"{'=' * 56}\n")
